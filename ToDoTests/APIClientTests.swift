...
//  ToDo
//
//  Created by Daniel Chapman on 10/3/17.
...
class APIClientTests: XCTestCase {
    ...
    func test_Login_UsesExpectedURL(){
        ...
        sut.loginUser(withName: "dasdom", password: "%&34", completion: completion)
        ...
        guard let expectedUsername = "dasdom".addingPercentEncoding(withAllowedCharacters: allowedCharacters) else { fatalError() }
    ...
    func test_Login_WhenJSONIsInvalid_ReturnsError(){
        ...
        let errorExpecation = expectation(description: "Error")
        var catchedError: Error? = nil
        sut.loginUser(withName: "Foo", password: "Bar") { (token, error) in
            catchedError = error
            errorExpecation.fulfill()
        }
